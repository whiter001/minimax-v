...
fn test_mcp_start_does_not_load_external_mcp_config() {
	...
	assert action == .continue_loop
	assert client.mcp_manager.servers.len == 0
	assert client.mcp_manager.get_all_tools().len == 0
	client.mcp_manager.stop_all()
}

fn test_init_mcp_external_loads_external_mcp_config() {
	mut client := new_api_client(default_config())
	config_home := os.join_path(os.temp_dir(), 'minimax-mcp-external-load-regression')
	os.mkdir_all(config_home) or { panic(err) }
	old_config_home := os.getenv('MINIMAX_CONFIG_HOME')
	os.setenv('MINIMAX_CONFIG_HOME', config_home, true)
	defer {
	...
	assert client.mcp_manager.servers[0].name == 'external'
	assert client.mcp_manager.get_all_tools().len == 0
	client.mcp_manager.stop_all()
}

fn test_init_mcp_external_skips_invalid_mcp_config() {
	mut client := new_api_client(default_config())
	config_home := os.join_path(os.temp_dir(), 'minimax-mcp-invalid-config-regression')
	os.mkdir_all(config_home) or { panic(err) }
	old_config_home := os.getenv('MINIMAX_CONFIG_HOME')
	os.setenv('MINIMAX_CONFIG_HOME', config_home, true)
	defer {
	...
	mcp_json := '{"servers":{"invalid":{"type":"stdio","command":"","args":[]}}}'
	os.write_file(os.join_path(config_home, 'mcp.json'), mcp_json) or { panic(err) }
