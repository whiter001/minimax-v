...
fn get_tools_schema_json() string {
	return '[' +
		...
		'{"name":"bash","description":"A persistent bash shell session. Working directory and environment variables are preserved between calls. Use this instead of run_command for multi-step operations.","input_schema":{"type":"object","properties":{"command":{"type":"string","description":"The bash command to execute"},"restart":{"type":"boolean","description":"Set to true to restart the bash session (reset cwd and env)"}},"required":["command"]}},' + '{"name":"read_file","description":"Read the contents of a file at the given path. Returns the full file content as text.","input_schema":{"type":"object","properties":{"path":{"type":"string","description":"The file path to read"}},"required":["path"]}},' + '{"name":"write_file","description":"Write content to a file at the given path. Creates the file if it does not exist, overwrites if it does.","input_schema":{"type":"object","properties":{"path":{"type":"string","description":"The file path to write to"},"content":{"type":"string","description":"The content to write to the file"}},"required":["path","content"]}},' + '{"name":"list_dir","description":"List the contents of a directory, showing files and subdirectories.","input_schema":{"type":"object","properties":{"path":{"type":"string","description":"The directory path to list"}},"required":["path"]}},' + '{"name":"run_command","description":"Run a shell command and return its output. Dangerous commands like rm -rf are blocked.","input_schema":{"type":"object","properties":{"command":{"type":"string","description":"The shell command to execute"}},"required":["command"]}},' + '{"name":"mouse_control","description":"Control mouse movement/click/scroll on desktop. Requires desktop control to be explicitly enabled.","input_schema":{"type":"object","properties":{"action":{"type":"string","description":"Action: move, click, or scroll","enum":["move","click","scroll"]},"x":{"type":"integer","description":"X coordinate for move/click"},"y":{"type":"integer","description":"Y coordinate for move/click"},"button":{"type":"string","description":"Mouse button for click","enum":["left","right","middle"]},"clicks":{"type":"integer","description":"Click count (1-5) for click action"},"delta":{"type":"integer","description":"Scroll delta for scroll action (positive/negative)"}},"required":["action"]}},' + '{"name":"keyboard_control","description":"Control keyboard input on desktop. Requires desktop control to be explicitly enabled.","input_schema":{"type":"object","properties":{"action":{"type":"string","description":"Action: type literal text or send raw keys","enum":["type","send"]},"text":{"type":"string","description":"Literal text for type action"},"keys":{"type":"string","description":"Raw SendKeys pattern for send action (e.g. ^l, {ENTER})"}},"required":["action"]}},' + '{"name":"capture_screen","description":"Capture desktop screenshot and save to a PNG file. Requires screen capture to be explicitly enabled.","input_schema":{"type":"object","properties":{"path":{"type":"string","description":"Output file path (.png). If omitted, temp path is used."},"x":{"type":"integer","description":"Region X (optional)"},"y":{"type":"integer","description":"Region Y (optional)"},"width":{"type":"integer","description":"Region width (optional, with height)"},"height":{"type":"integer","description":"Region height (optional, with width)"}},"required":[]}},' + '{"name":"screen_analyze","description":"Capture screen (or use existing image path) then call MCP understand_image to analyze it.","input_schema":{"type":"object","properties":{"image_path":{"type":"string","description":"Existing image file path. If omitted, screen is captured first."},"image_source":{"type":"string","description":"Alias of image_path for understand_image compatibility."},"prompt":{"type":"string","description":"Analysis instruction sent to understand_image"},"x":{"type":"integer","description":"Region X for capture when image_path omitted"},"y":{"type":"integer","description":"Region Y for capture when image_path omitted"},"width":{"type":"integer","description":"Region width for capture when image_path omitted"},"height":{"type":"integer","description":"Region height for capture when image_path omitted"}},"required":[]}},' +
		...
		'{"name":"session_note","description":"Persistent memory across sessions. Use this to save important context, decisions, project info, or user preferences that should be remembered in future sessions. Actions: read (get all notes), write (overwrite notes), append (add to notes).","input_schema":{"type":"object","properties":{"action":{"type":"string","description":"The action: read, write, or append","enum":["read","write","append"]},"content":{"type":"string","description":"Content to write/append (required for write and append actions)"}},"required":["action"]}},' + '{"name":"task_done","description":"Signal that the current task is complete. Call this tool when you have finished the user\'s request and want to provide a final summary. Before calling this, consider if you should update session_note with mid-project decisions, progress, or temporary constraints to help future sessions.","input_schema":{"type":"object","properties":{"result":{"type":"string","description":"A concise summary of what was accomplished"}},"required":["result"]}},' + '{"name":"grep_search","description":"Search for a pattern in file contents using regex. Searches recursively in the given directory. Returns matching lines with file paths and line numbers.","input_schema":{"type":"object","properties":{"pattern":{"type":"string","description":"The regex pattern to search for"},"path":{"type":"string","description":"Directory or file to search in (default: current workspace)"},"include":{"type":"string","description":"Only search files matching this glob pattern (e.g. *.v, *.go)"}},"required":["pattern"]}},' + '{"name":"find_files","description":"Find files by name pattern. Searches recursively in the given directory. Returns matching file paths.","input_schema":{"type":"object","properties":{"pattern":{"type":"string","description":"File name pattern with wildcards (e.g. *.v, test_*, *.{js,ts})"},"path":{"type":"string","description":"Directory to search in (default: current workspace)"}},"required":["pattern"]}},' + '{"name":"sequentialthinking","description":"A tool for structured step-by-step reasoning. Use this to break down complex problems, plan approaches, analyze options, or reason through multi-step tasks before taking action. Each thought can be a revision or branch of a previous thought. The thought process is displayed to the user.","input_schema":{"type":"object","properties":{"thought":{"type":"string","description":"Your current thinking step content"},"thought_number":{"type":"integer","description":"Current thought number (starting from 1)"},"total_thoughts":{"type":"integer","description":"Estimated total number of thoughts needed (can adjust as thinking progresses)"},"next_thought_needed":{"type":"boolean","description":"Whether another thought step is needed after this one"},"is_revision":{"type":"boolean","description":"Whether this revises a previous thought"},"revises_thought":{"type":"integer","description":"If is_revision=true, which thought number this revises"},"branch_from_thought":{"type":"integer","description":"If branching, which thought to branch from"}},"required":["thought","thought_number","total_thoughts","next_thought_needed"]}},' + '{"name":"json_edit","description":"View or edit JSON files using dot-notation paths. Actions: view (show JSON content, optionally at a path), set (set or update a value at a path), add (add a new key/value or append to array), remove (delete a key or array element). Examples: path=\\"server.port\\", path=\\"items[0].name\\".","input_schema":{"type":"object","properties":{"action":{"type":"string","description":"The action: view, set, add, or remove","enum":["view","set","add","remove"]},"file":{"type":"string","description":"Path to the JSON file"},"path":{"type":"string","description":"Dot-notation path to the target (e.g. server.port, items[0]). Empty for root."},"value":{"type":"string","description":"(set/add only) The value to set. Will auto-detect type: numbers, booleans, null, or strings."}},"required":["action","file"]}},' +
