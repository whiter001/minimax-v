...
- 状态通过 `ApiClient` 和局部 manager 传递，例如 MCP 管理器、Bash 会话、技能注册表、TODO 管理器。
- 编译不再需要额外的全局变量开关。
...
在入口早期先分流 Cron 子命令，避免进入普通聊天启动流程。`cron dashboard` 通过 veb 在本地提供只读页面，并把任务快照和执行历史镜像到 SQLite，便于浏览任务状态。

## 关键状态

### 全局变量

当前实现不再依赖进程级隐式全局变量；运行时状态由显式对象持有，例如 `ApiClient`、`CheckpointManager` 和 `TimerManager`。
